...
const createSampleLeads = async () => {
  try {
    ...
    console.log(`Created ${sampleLeads.length} sample leads (2 FTD, 2 Filler, 2 Cold)`);
  } catch (error) {
...
const createSamplePerformance = async () => {
  try {
    ...
    console.log(`Created ${performanceData.length} performance records for ${agents.length} agents`);
  } catch (error) {
